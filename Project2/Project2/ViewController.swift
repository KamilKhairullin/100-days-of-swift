...
//  Project2
//
//  Created by Kamil on 29.09.2021.
//
...
class ViewController: UIViewController {
    ...
    func askQuestion(action: UIAlertAction! = nil) {
        ...
        title = "Your score is: \(score) | " + countries[correctAnswer].uppercased()
    }
    ...
    @IBAction func buttonTapped(_ sender: UIButton) {
        ...
        if questionsAsked % 10 == 0 {
            let ac = UIAlertController(title: "Final result", message: "Your score is \(score)", preferredStyle: .alert)
            ac.addAction(UIAlertAction(title: "Try again", style: .default, handler: askQuestion))
            present(ac, animated: true)
            score = 0
            questionsAsked = 0
        } else if title == "Wrong" {
            let ac = UIAlertController(title: title, message: "Correct answer is \(countries[correctAnswer].uppercased())", preferredStyle: .alert)
